...
# 训练数据集
training_set = [[(1, 2), 1], [(2, 3), 1], [(1, 1), 1], [(2, 2), 1],
                [(1, 1.5), 1], [(1.5, 2), 1], [(1, 2.5), 1], [(1.5, 5), 1],
                [(3, 1), -1], [(4, 2), -1], [(4, 2.5), -1], [(5, 2), -1],
                [(6, 2), -1], [(5, 2.5), -1], [(4, 2.2), -1], [(5, 3), -1]]

w = [0, 0]  # 参数初始化
b = 0
history = []  # 用来记录每次更新过后的w,b
historyLine = []  # 用来记录每次更新过后的分割线
...
def update(item):
    ...
    historyLine.append('-' + str(w[0] / w[1]) + 'x-' + str(b / w[1]))
